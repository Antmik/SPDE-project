...
cdef class non_markovian_FH_binary_reaction:
    ...
    cdef object Reaction1
    cdef object Reaction2
    cdef np.ndarray A_vec1
    cdef np.ndarray B_vec1
    cdef np.ndarray b_vec1
    cdef np.ndarray A_vec2
    cdef np.ndarray B_vec2
    cdef np.ndarray b_vec2
    cdef int brownian_bridge_calls
    cdef np.ndarray S
    
    def __cinit__(self, str boundaries, np.ndarray[np.float64_t, ndim=1] x , object energy_derivative , np.ndarray[np.float64_t, ndim=1] A_vec1, np.ndarray[np.float64_t, ndim=1] B_vec1, np.ndarray[np.float64_t, ndim=1] A_vec2, np.ndarray[np.float64_t, ndim=1] B_vec2, object Reaction1, object Reaction2 , float open_boundary_value =0.0):
        
        if ( len(A_vec1)!=len(B_vec1)):
            raise TypeError('Error in the parameters: A_k and B_k')
        else:
            self.A_vec1= A_vec1
            self.B_vec1= B_vec1
            self.b_vec1= np.zeros(len(A_vec1))#  sqrt( np.multiply( A_vec, -B_vec ))
            
            #find b_k vector
            if ( np.all(self.B_vec1) != 0 and np.all(self.A_vec1) != 0):
                self.b_vec1=np.array( noiseCoeff(len(A_vec1),A_vec1,-B_vec1) )
                print(self.A_vec1, self.B_vec1, self.b_vec1)

        if ( len(A_vec2)!=len(B_vec2)):
            raise TypeError('Error in the parameters: A_k and B_k')
        else:
            self.A_vec2= A_vec2
            self.B_vec2= B_vec2
            self.b_vec2= np.zeros(len(A_vec2))#  sqrt( np.multiply( A_vec, -B_vec ))
            
            #find b_k vector
            if ( np.all(self.B_vec2) != 0 and np.all(self.A_vec2) != 0):
                self.b_vec2=np.array( noiseCoeff(len(A_vec2),A_vec2,-B_vec2) )
                print(self.A_vec2, self.B_vec2, self.b_vec2)
    ...
    def step(self, np.ndarray[np.float64_t, ndim=1] rho, np.ndarray[np.float64_t, ndim=1] momentum, np.ndarray[np.float64_t, ndim=2] S,  np.ndarray[np.float64_t,ndim=1] eta, A_vec, B_vec, b_vec , float dt, brownian_bridge_calls, object Reaction_func, np.ndarray[np.float64_t, ndim=1] rho_other ):
    
        dim=rho.shape[0]
        n_S= len(A_vec)
        S_tmp=np.zeros([dim,n_S],dtype=np.float64)
        S_output =np.zeros([dim,n_S],dtype=np.float64)
        
        mu_rho= compute_mu_rho1( self.x, momentum, self.boundaries) + Reaction_func(rho, rho_other )
        ...
        for i_S in range (n_S):
            mu_S = compute_mu_S(self.x, S[:,i_S], momentum, A_vec[i_S], B_vec[i_S], self.boundaries)
            sigma_S = compute_sigma_S(self.x, rho, b_vec[i_S], self.boundaries)
            S_tmp[:,i_S]= time_integrator(S[:,i_S], mu_S, sigma_S, eta, dt)
        
            
        #Corrector
        mu_rho= compute_mu_rho2( self.x, momentum_tmp, self.boundaries) + Reaction_func(rho_tmp, rho_other )
        rho_output= 0.5*(rho + time_integrator(rho_tmp, mu_rho, np.zeros(dim), np.zeros(dim), dt) )
        
        mu_momentum= compute_mu_momentum2(self.x, rho_tmp, momentum_tmp, np.sum(S_tmp, axis=1) , self.energy_derivative, self.boundaries)
        momentum_output= 0.5*(momentum +  time_integrator(momentum_tmp, mu_momentum, np.zeros(dim) , np.zeros(dim), dt) )
        
        for i_S in range (n_S):
            mu_S = compute_mu_S(self.x, S_tmp[:, i_S], momentum_tmp, A_vec[i_S], B_vec[i_S], self.boundaries)
            sigma_S = compute_sigma_S(self.x, rho_tmp, b_vec[i_S], self.boundaries)
            S_output[:,i_S]= 0.5*(S[:,i_S] + time_integrator(S_tmp[:,i_S], mu_S, sigma_S, eta, dt) )
        
        if ( any ( [ value < 0 or isnan(value) or isinf(value) for value in rho_output] ) ):
            brownian_bridge_calls +=1
            rho_output, momentum_output, S_output = brownian_bridge(rho, momentum, S, eta, dt, self.x , self.energy_derivative , A_vec, B_vec, b_vec, self.boundaries)
    
        return rho_output, momentum_output,S_output,brownian_bridge_calls
    

    
    def __call__(self, np.ndarray[np.float64_t, ndim=1] rho1, np.ndarray[np.float64_t, ndim=1] momentum1, np.ndarray[np.float64_t, ndim=3] eta1, np.ndarray[np.float64_t, ndim=1] rho2, np.ndarray[np.float64_t, ndim=1] momentum2, np.ndarray[np.float64_t, ndim=3] eta2, float dt, int n_steps, n_traj=1):
        
        
        dim=rho1.shape[0]
        n_S= len(self.A_vec1)
        
        rho_output1= np.zeros([dim,n_steps,n_traj],dtype=np.float64)
        momentum_output1= np.zeros([dim,n_steps,n_traj],dtype=np.float64)
        S_output1=np.zeros([dim,n_S,n_steps,n_traj],dtype=np.float64)
        
        rho_output2= np.zeros([dim,n_steps,n_traj],dtype=np.float64)
        momentum_output2= np.zeros([dim,n_steps,n_traj],dtype=np.float64)
        S_output2=np.zeros([dim,n_S,n_steps,n_traj],dtype=np.float64)
        
        #Initialize S
        S1=np.zeros([dim,n_S],dtype=np.float64)
        #for i_S in range (n_S):
            #if (self.B_vec1[i_S] != 0):
            #    S1[:,i_S]= self.b_vec1[i_S] / np.sqrt( -self.B_vec1[i_S]) *np.random.randn(dim)

        S2=np.zeros([dim,n_S],dtype=np.float64)
        #for i_S in range (n_S):
            #if (self.B_vec2[i_S] != 0):
            #   S2[:,i_S]= self.b_vec2[i_S] / np.sqrt( -self.B_vec2[i_S]) *np.random.randn(dim)
                
        
        brownian_bridge_calls=0
        for i_traj in range (n_traj):
            
            rho_output1[:,0,i_traj], momentum_output1[:,0,i_traj], S_output1[:,:,0,i_traj], brownian_bridge_calls = self.step(rho1, momentum1, S1, eta1[:,0,i_traj] , self.A_vec1, self.B_vec1, self.b_vec1 , dt, brownian_bridge_calls, self.Reaction1, rho2)

            rho_output2[:,0,i_traj], momentum_output2[:,0,i_traj], S_output2[:,:,0,i_traj], brownian_bridge_calls = self.step(rho2, momentum2, S2, eta2[:,0,i_traj] ,self.A_vec2, self.B_vec2, self.b_vec2 ,dt, brownian_bridge_calls, self.Reaction2, rho1)
            
            for i in range (1,n_steps):
                rho_output1[:,i,i_traj], momentum_output1[:,i,i_traj], S_output1[:,:,i,i_traj], brownian_bridge_calls = self.step(rho_output1[:,i-1,i_traj], momentum_output1[:,i-1,i_traj], S_output1[:,:,i-1,i_traj], eta1[:,i,i_traj] ,self.A_vec1, self.B_vec1, self.b_vec1, dt, brownian_bridge_calls, self.Reaction1, rho_output2[:,i-1,i_traj])

                rho_output2[:,i,i_traj], momentum_output2[:,i,i_traj], S_output2[:,:,i,i_traj], brownian_bridge_calls = self.step(rho_output2[:,i-1,i_traj], momentum_output2[:,i-1,i_traj], S_output2[:,:,i-1,i_traj], eta2[:,i,i_traj] ,self.A_vec2, self.B_vec2, self.b_vec2, dt, brownian_bridge_calls, self.Reaction2, rho_output1[:,i-1,i_traj])

        print("Brownian bridge calls: %d" %brownian_bridge_calls)
                            
        return rho_output1,  momentum_output1, S_output1, rho_output2,  momentum_output2, S_output2
