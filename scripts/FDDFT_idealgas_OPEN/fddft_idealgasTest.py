...
if __name__ == '__main__':
    ...
    dt= 0.1*dx*dx #1* 10**(-3)
    eta= 0*np.random.randn(n,n_steps,n_traj)
    
    #Initialize fddft class
    my_fddft= fddft(boundaries, x , energy_derivative, open_boundary_value =1.0)
    
    t0 = time.time()
    rhoEM =  my_fddft(rho0, eta, dt, n_steps,space_method='UW', time_method='EM', theta=0.5, n_traj=n_traj)
    t1 = time.time()
    total_timeEM=t1-t0
    print("Time:%f" %total_timeEM)

    print(np.sum(rho0))
    print( np.sum(rhoEM[:,-1,-1]) )
